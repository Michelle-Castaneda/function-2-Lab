...
// CODE HERE
const uniq = (arr,cb) => {
  for(let i=0; i<arr.length;i++) {    // Iterate over array elements
    for(let j = i +1; j<arr.length;j++) {
       if(arr[i] === arr[j]){
        arr.splice(j,1); //if duplicate found remove it from the array
...
const addNUMBER = addingFactory(9)
...
// CODE HERE 
function getUserById(usersarr,id,cb) {
  for(let i = 0; i < usersarr.length; i++) {
    if(usersarr[i].id === id)
      return cb(usersarr[i])
  }
